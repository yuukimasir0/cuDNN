...
|I/O 精度(FP16, FP32, INT8, or FP64)|FP16<br><br>FP32 - <br>&emsp;Ampereアーキテクチャ以前では`CUDNN_TENSOROP_MATH_ALLOW_CONVERSION`を設定することで利用可能。<br>&emsp;NVIDIA Ampere アーキテクチャではデフォルトでTF32(Tensor Float 32)が使用されます。<br><br>INT8 - <br>&emsp;INT8はdgradとwgradではサポートされていません。<br>&emsp;INT8での3D畳み込みはバックエンドAPIのみでサポートされています。詳しくは`cudnnConvolutionForward()`の表を参照してください。 |
